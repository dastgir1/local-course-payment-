define('local_course_carousel/carousel', [], function() {
    return {
        init: function() {
            ...
            var cards = Array.prototype.slice.call(carouselInner.querySelectorAll('.card'));
            if (cards.length === 0) {
                return;
            }

            var uniqueCards = cards.filter(function(card, index, list) {
                var image = card.querySelector('img');
                var imageSrc = image ? image.getAttribute('src') : '';

                return list.findIndex(function(item) {
                    var itemImage = item.querySelector('img');
                    return (itemImage ? itemImage.getAttribute('src') : '') === imageSrc;
                }) === index;
            });
            ...
            uniqueCards.slice(0, visibleCards).forEach(function(card) {
                cardWrapper.appendChild(card.cloneNode(true));
            });
            ...
            var moveSlider = function() {
                var firstCard = cardWrapper.querySelector('.card');
                if (!firstCard) {
                    return;
                }

                var cardWidth = firstCard.offsetWidth;
                var gap = parseFloat(getComputedStyle(cardWrapper).gap) || 0;
            ...
            cardWrapper.addEventListener('transitionend', function() {
